...
#'   \item{\code{output}}{(Mandatory) Entry to tell \code{rmarkdown} to render via \code{modeloLEA};
#'   must be \code{modeloLEA::modeloLEA}.}
...
modeloLEA <- function(..., keep_tex = TRUE, citation_package = 'natbib') {
    ...
    for (f in c("modeloLEA.cls", "jss.bst"))
        if (!file.exists(f))
            file.copy(system.file("rmarkdown", "templates", "pdf", "skeleton", f, package="modeloLEA"),
                      ".")
